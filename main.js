...
const { createApp } = Vue
  ...
  createApp({
    ...
    methods: {
      insert(){
        const nuovoOggetto= {text: `${this.taskValue}`, done: false}
        if(nuovoOggetto.text !== ''){
            this.tasks.unshift(nuovoOggetto)
            this.taskValue= ''
        }
      },
      delet(index) {
...
//   piu tardi proviamo ad impostare tutto a false 
// ed a cambiare il done
// this.tasks[index].done = !this.tasks[index].done; 